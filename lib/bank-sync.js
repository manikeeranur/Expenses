...
export async function syncBankLinkData(link, session) {
    ...
    for (const fipAccount of accounts) {
      ...
      for (const t of transactions) {
        ...
        try {
          ...
          // Unique index race: another concurrent sync inserted this same
          // bank reference first — treat it as a duplicate, not a failure.
  ...
  console.log(
    `[Setu] Sync complete for bankLinkId=${link._id}: imported=${imported} duplicates=${duplicates} fips=${fips.length}`
  );
