...
// A UPI payment that's initiated/pending/cancelled hasn't actually left the
// wallet yet, so it's excluded from money totals until confirmed paid.
// Non-UPI transactions have paymentStatus === null and are unaffected.
const SETTLED_ONLY = { paymentStatus: { $nin: UNSETTLED_UPI_STATUSES } };
...
export async function getCategories(userId) {
  await dbConnect();
  const categories = await Category.find({ userId }).sort({ order: 1, budget: -1 }).lean();
...
export async function getLendings(userId) {
  await dbConnect();
  const lendings = await Lending.find({ userId }).sort({ order: 1, dateGiven: -1 }).lean();
...
// Flattens every borrower's embedded payments into one feed, newest first —
// there's no separate payments collection, so this reads all lendings and
// merges their payments[] in JS rather than a $unwind aggregation.
export async function getRecentLendingPayments(userId, limit = 5) {
  await dbConnect();
  const lendings = await Lending.find({ userId }).select("borrower payments").lean();
  const rows = [];
  for (const l of lendings) {
    for (const p of l.payments || []) {
      rows.push({
        lendingId: l._id.toString(),
        borrower: l.borrower,
        date: p.date,
        amount: p.amount,
        type: p.type,
...
export async function getActiveQrCode(userId) {
  await dbConnect();
  const qr = await Payment.findOne({ userId, kind: "qr", status: "created" }).sort({ createdAt: -1 }).lean();
  return qr ? plain(qr) : null;
}

export async function getQrPayments(userId, qrCodeId) {
  await dbConnect();
  const payments = await Payment.find({ userId, kind: "qr", razorpayQrCodeId: qrCodeId, status: "paid" })
    .sort({ createdAt: -1 })
    .lean();
  return plain(payments);
}

export async function getLatestBankLink(userId) {
  await dbConnect();
  const link = await BankLink.findOne({ userId }).sort({ createdAt: -1 }).lean();
  return link ? plain(link) : null;
}

export async function getBankLinks(userId) {
  await dbConnect();
  const links = await BankLink.find({ userId }).sort({ createdAt: -1 }).lean();
  return plain(links);
}
...
export async function getMonthSpendByCategory(userId, date = new Date()) {
  ...
  const rows = await Transaction.aggregate([
    { $match: { userId: oid(userId), type: "expense", date: { $gte: start, $lt: end }, ...SETTLED_ONLY } },
...
export async function getDashboardSummary(userId, date = new Date()) {
  ...
  const [totals, prevTotals] = await Promise.all([
    Transaction.aggregate([
      { $match: { userId: oid(userId), date: { $gte: start, $lt: end }, ...SETTLED_ONLY } },
      { $group: { _id: "$type", total: { $sum: "$amount" } } },
    ]),
    Transaction.aggregate([
      { $match: { userId: oid(userId), date: { $gte: prevStart, $lt: prevEnd }, ...SETTLED_ONLY } },
      { $group: { _id: "$type", total: { $sum: "$amount" } } },
    ]),
  ]);
...
export async function getMonthlyTrend(userId, months = 6) {
  ...
  const rows = await Transaction.aggregate([
    { $match: { userId: oid(userId), type: "expense", date: { $gte: start }, ...SETTLED_ONLY } },
...
export async function getMonthlyIncomeExpense(userId, months = 6) {
  ...
  const rows = await Transaction.aggregate([
    { $match: { userId: oid(userId), date: { $gte: start }, ...SETTLED_ONLY } },
...
export async function getCategoryStats(userId, categoryId, date = new Date()) {
  ...
  const match = { userId: oid(userId), type: "expense", categoryId: oid(categoryId), ...SETTLED_ONLY };
  ...
  for (const t of transactions) {
    ...
    byDate[key].push({
      ...
      categoryId: t.categoryId
        ? { _id: t.categoryId._id.toString(), name: t.categoryId.name, icon: t.categoryId.icon, color: t.categoryId.color }
        : null,
      accountId: t.accountId ? { _id: t.accountId._id.toString(), name: t.accountId.name } : null,
    });
  }
  return plain(byDate);
}
