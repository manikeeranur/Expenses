...
export async function startBankLinkConsent(mobileNumber, aa, vua) {
  ...
  let consent;
  try {
    consent = await createConsent(vua, `${baseUrl}/accounts/link/complete`);
  } catch (err) {
...
export async function checkBankLinkStatus(bankLinkId) {
  ...
  try {
    ...
    return { success: true, consentStatus: link.consentStatus, dataSessionStatus: link.dataSessionStatus };
  } catch (err) {
