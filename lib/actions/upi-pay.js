...
// that name here instead of whatever the QR itself says.
export async function lookupPayeeName(upiId) {
  const userId = await requireUserId();
  const cleanUpiId = sanitizeUpiText(upiId, 100);
  if (!isValidUpiId(cleanUpiId)) return { name: null };
...
export async function initiateUpiPayment(prevState, formData) {
  ...
  if (UPI_DEBUG) console.log("ORIGINAL_QR_PAYLOAD", scannedUri || null);
  ...
  // Full field-by-field trail, including params this app doesn't otherwise
  // know about (mode, purpose, orgid, sign, refUrl, …), plus an explicit
  // diff so a mismatch between what was scanned and what's launched is
  // impossible to miss.
  const allOriginalParams = scannedUri ? getAllUpiParams(scannedParsed?.raw || scannedUri) : {};
  const allFinalParams = getAllUpiParams(upiUri);
  const diff = scannedUri ? diffUpiUris(scannedParsed?.raw || scannedUri, upiUri) : null;
  ...
  if (UPI_DEBUG) {
    console.log("FINAL_UPI_URI", upiUri);
    console.log("UPI_PARAM_DIFF", diff);
  }
