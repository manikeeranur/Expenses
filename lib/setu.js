...
async function getSetuToken() {
  ...
  console.log("[Setu] Requesting auth token");
...
export async function createConsent(vua, redirectUrl) {
  ...
  console.log(`[Setu] Creating consent for vua=${vua}`);
  const consent = await setuFetch("/v2/consents", {
    method: "POST",
    body: {
  ...
  console.log(`[Setu] Consent created id=${consent?.id} status=${consent?.status}`);
  return consent;
}
...
export async function createDataSession(consentId, dataRange) {
  console.log(`[Setu] Creating data session for consentId=${consentId}`);
