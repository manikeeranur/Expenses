...
async function setuFetch(path, { method = "GET", body } = {}) {
  ...
  if (!res.ok) {
    throw new Error(json?.message || json?.errors?.[0]?.detail || `Setu request failed (${res.status})`);
  }
  return json;
}
...
export async function createConsent(vua, redirectUrl) {
  return setuFetch("/consents", {
    ...
    body: {
      ...
      vua,
      dataRange: { from: "2020-01-01T00:00:00Z", to: new Date().toISOString() },
      context: [],
      redirectUrl,
    },
  });
...
export async function getConsentStatus(consentId) {
  return setuFetch(`/consents/${consentId}`);
}
