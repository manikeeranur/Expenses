...
export default function TransactionForm({ action, categories, accounts, defaults, cancelHref, submitLabel }) {
  ...
  return (
    <form action={formAction} className="px-5 pb-10 pt-6">
      ...
      <div className="mt-4">
        <input
          ...
          required
          className="w-full rounded-2xl bg-surface px-4 py-3.5 text-sm shadow-sm shadow-black/[0.03] outline-none placeholder:text-muted"
        />
      </div>
      ...
      <button
        type="submit"
        disabled={pending}
        className="mt-6 flex w-full items-center justify-center rounded-2xl bg-primary py-3.5 text-sm font-semibold text-white shadow-lg shadow-primary/25 disabled:opacity-60"
      >
