...
const STATUS_COPY = {
  ...
  INITIATED: { icon: Clock, label: "Consent request sent — waiting for your approval", tone: "text-warning" },
  ACTIVE: { icon: Check, label: "Consent approved — fetching your data", tone: "text-success" },
  REJECTED: { icon: XCircle, label: "Bank account connection was cancelled or rejected.", tone: "text-danger" },
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center px-8 text-center">
      ...
      <p className={`mt-1 text-sm ${synced ? "text-success" : status.tone}`}>
        {synced ? "Bank account connected successfully." : status.label}
      </p>
